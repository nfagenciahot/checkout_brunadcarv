...
function githubSettings() {
  ...
  if (!owner) throw new Error('GITHUB_OWNER não configurado na Vercel.');
  if (!repo) throw new Error('GITHUB_REPO não configurado na Vercel.');
...
async function githubRead(relPath) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`GitHub GET ${relPath}: ${res.status} ${detail.slice(0, 300)}`);
  }
...
async function githubWrite(relPath, bytes, message = 'Atualização pelo painel') {
  const cfg = githubSettings();
...
async function githubDelete(relPath, message = 'Remover arquivo pelo painel') {
  const cfg = githubSettings();
  const url = `https://api.github.com/repos/${cfg.owner}/${cfg.repo}/contents/${encodeURI(relPath)}`;
  ...
  if (!res.ok) {
    const detail = await res.text();
    throw new Error(`GitHub DELETE ${relPath}: ${res.status} ${detail.slice(0, 300)}`);
  }
